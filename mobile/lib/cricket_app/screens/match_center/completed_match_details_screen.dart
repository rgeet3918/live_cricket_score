...
/// Completed Match Details Screen
/// Shows details for completed/finished matches
class CompletedMatchDetailsScreen extends ConsumerStatefulWidget {
  final MatchModel match;

  const CompletedMatchDetailsScreen({super.key, required this.match});

  @override
  ConsumerState<CompletedMatchDetailsScreen> createState() =>
      _CompletedMatchDetailsScreenState();
}

class _CompletedMatchDetailsScreenState
    extends ConsumerState<CompletedMatchDetailsScreen> {
  ...
  void initState() {
    super.initState();
    FirebaseAnalyticsService.logScreenView('completed_match_details_screen');
    _selectedTab = 0;
  ...
  Widget _buildMatchCard() {
    ...
    // Calculate CRR (Current Run Rate) for team 1
    String crr = '-';
    if (score1 != null && score1.o > 0) {
      crr = (score1.r / score1.o).toStringAsFixed(2);
    }

    // Calculate RRR (Required Run Rate) for team 2
    String rrr = '-';
      ...
      child: Column(
        ...
        children: [
          ...
          if (match.status.isNotEmpty) ...[
            const SizedBox(height: 12),
            _buildInfoRow('Result', match.status),
          ],
        ],
      ),
    );
